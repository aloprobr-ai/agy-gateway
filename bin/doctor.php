...
// ------------------------------------------------------- доступ к шлюзу
// Спрашиваем у самого Auth — тот же признак, что показывает /health
// в поле auth_required. Сами ключи здесь не печатаются, только сколько их.
require $root . '/src/Keys.php';
require $root . '/src/Auth.php';

$builtin = count((array) ($cfg['api_keys'] ?? []));
$issued = count(Keys::active());

if (Auth::required($cfg)) {
    $say('шлюз закрыт клиентским ключом', true,
        "в config.php: {$builtin}, выдано на /keys: {$issued}");
    if ($builtin === 0 && $issued === 0) {
        // Ключи когда-то выдавались, но сейчас все отключены: защита осталась,
        // а подойти к ней нечем. Молча это выглядит как «шлюз сломался».
        $say('есть чем войти', false, 'все выданные ключи отключены',
            "включите нужный на странице /keys\n"
            . "или впишите ключ в 'api_keys' в config.php");
    }
} else {
    $say('шлюз закрыт клиентским ключом', null, 'нет — ответит любому, кто знает адрес',
...
// Сети здесь не касаемся намеренно: проверка обстановки должна отвечать
// быстро и работать там, где интернета нет вовсе.
echo "Посмотреть, не вышло ли обновление шлюза: php bin/update.php\n";
exit($problems === 0 ? 0 : 1);

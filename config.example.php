...
return [
    ...
    'cli' => [
        ...
        // Системное сообщение клиента — в GEMINI.md каталога беседы, а не
        // текстом в промпт: agy ставит такие правила выше собственного
        // промпта. false — по-старому. Длиннее 20 000 байт всё равно уходит текстом.
        'system_as_rules' => true,

        // Покупной прокси для перехватчика deploy/agy-mitm (README, «Без
        // промпта agy»): его читает служба agy-mitm, а в 'env' тогда стоит
        // HTTPS_PROXY = http://127.0.0.1:18080. Без перехватчика не нужен.
    ...
    // Пустой массив = вход без ключа, но только если ключи не выдавались и на
    // странице /keys: там они работают наравне с этими. Требует шлюз ключ или
    // нет, видно в /health, поле auth_required — оно показывает то, что решит
    // сам Auth, а не содержимое этого массива.
    // Без ключа на своей машине удобно, но шлюз тогда нельзя открывать наружу:
    // любой, кто до него достучится, получит и вашу учётную запись
    // Antigravity, и право запускать CLI.
    'api_keys' => [
    ...
    'admin' => [
        // С каких адресов видна страница. Пусто — управление выключено совсем.
        'ips' => ['127.0.0.1', '::1'],
        // Токен управления: пароль на выдачу и отзыв ключей (и X-Admin-Token).
        // Это НЕ ключ доступа к API. Пусто — при первом запуске bin/serve.sh
        // или bin/serve.ps1 шлюз придумает его сам (tk-...), впишет сюда
        // и покажет. Без них: php bin/admin-token.php. Можно и свою строку.
        'token' => '',
    ],
    ...
    // ------------------------------------------------------------ обновления
    // Откуда шлюз берёт свежий код: php bin/update.php
    'update' => [
        'repo' => 'aloprobr-ai/agy-gateway',
        'branch' => 'main',
        // Ключ доступа GitHub. Нужен, только пока репозиторий закрыт:
        // у закрытого API отвечает 404 без ключа.
        // Можно не писать сюда, а передать переменной окружения
        // AGY_UPDATE_TOKEN — секрету в файле не место, если можно иначе.
        'token' => '',
    ],
